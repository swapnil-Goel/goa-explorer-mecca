...
const LOCATIONS = [
  ...
  { id: 7,  name: "Baga Beach",           x: 19.6, y: 36.9 },
  { id: 8,  name: "Calangute Beach",      x: 17.7, y: 40.3 },
  { id: 9,  name: "Candolim Beach",       x: 26.0, y: 43.5 },
  { id: 10, name: "Panjim",               x: 43.9, y: 38.8 },
  { id: 11, name: "Fontainhas",           x: 40.3, y: 42.7 },
  ...
  { id: 19, name: "Agonda Beach",         x: 36.5, y: 83.0 },
  { id: 20, name: "Palolem Beach",        x: 40.5, y: 87.8 },
  { id: 21, name: "Dudhsagar Waterfalls", x: 76.1, y: 57.9 },
...
// ═══════════════════════════════════════
// ═══════════════════════════════════════
// MODAL – TEAM_MECCA (Image)
// ═══════════════════════════════════════

const TEAM_MECCA_IMAGE =
  "/Team-Image.avif";

const TEAM_MECCAModal = ({ onClose }) => {
  return (
    <ModalWrapper
      onClose={onClose}
      title="TEAM_MECCA"
    ...
    >
      <div className="mt-4">
        <a
          href={TEAM_MECCA_IMAGE}
          target="_blank"
          rel="noopener noreferrer"
        >
          <img
            src={TEAM_MECCA_IMAGE}
            alt="TEAM_MECCA"
...
const userEmail = user?.email || '';
  ...
  const [couponsUnlocked, setCouponsUnlocked] = useState(0);
...
const [couponRemainingCount, setCouponRemainingCount] = useState(TOTAL_COUPONS);
  ...
  const [modal, setModal] = useState(null); // 'TEAM_MECCA' | 'howtoplay' | 'aboutmecca'
...
setTotalCoupons(status.total);
  ...
  >
    THE MARKETING CLUB OF GIM
        ...
        <div className="flex items-center gap-2">
          <button className="nav-btn" onClick={() => setModal('TEAM_MECCA')}>
            <Trophy size={14} />
            <span>TEAM_MECCA</span>
          </button>
...
<div className="flex-1 flex items-center justify-center px-4 py-6">
  <img
    src="/mecca-logo1.png"
    alt="MECCA"
    className="w-full max-w-[4000px] object-contain opacity-100"
  />
</div>
          </div>
        </aside>
...
>
  ...
  >
    Explore Goa • Discover More
      ...
      {/* ═══ MODALS ═══ */}
      {modal === 'TEAM_MECCA' && <TEAM_MECCAModal onClose={() => setModal(null)} />}
